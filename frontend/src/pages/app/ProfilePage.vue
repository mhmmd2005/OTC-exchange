...
const completionItems = computed(() => {
  ...
  return [
    ...
    {label: 'اطلاعات هویتی', complete: profile.value.identityVerified, icon: 'verify'},
...
<template>
  <div class="page profile-page">
    <PageHeader
        title="حساب کاربری"
        description="اطلاعات هویتی، وضعیت حساب و راه‌های ارتباطی خود را مرور کنید."
    >
      <template #actions>
        <AppButton
            variant="secondary"
    ...
    <template v-if="loading">
      <AppCard padding="lg">
        <div class="profile-skeleton">
          <AppSkeleton width="5rem" height="5rem" radius="1.5rem"/>
          <div>
            <AppSkeleton width="10rem" height="1.5rem"/>
            <AppSkeleton width="15rem" height=".9rem"/>
          </div>
        </div>
      </AppCard>
...
.email-field :deep(.field__control) {
  position: relative;
}

.email-field :deep(.field-edit) {
  position: absolute;
...
.profile-skeleton :deep(.skeleton) {
  max-width: 100%;
}
...
.feedback {
  ...
  border: 1px solid rgba(53, 201, 149, .22);
...
.level-card-mini {
  ...
  min-width: 14rem;
  padding: var(--space-3) var(--space-4);
  border: 1px solid rgba(221, 183, 110, .2);
  border-radius: var(--radius-lg);
  background: var(--color-gold-soft);
}

.level-card-mini > span {
  display: grid;
  width: 2.4rem;
  height: 2.4rem;
  border-radius: .75rem;
...
.identity-grid :deep(.field__control:has(input[readonly])) {
  background: var(--color-surface-2);
}

.identity-grid :deep(input[readonly]) {
  color: var(--color-text-secondary);
  cursor: default;
}
...
.completion-card li > span {
  display: flex;
  align-items: center;
  gap: var(--space-2);
}

.completion-card li.complete {
  color: var(--color-text-secondary);
}

.completion-card li.complete > svg {
  color: var(--color-success);
}
...
.avatar-wrap, .hero-copy, .level-card-mini {
  position: relative;
  z-index: 1;
}
...
.field-edit:focus-visible, .level-card-mini a:focus-visible {
...
@media (max-width: 399px) {
  ...
  .locked-label {
    width: 100%;
    padding-inline-start: calc(2.7rem + var(--space-3));
  }

  .identity-help :deep(.app-button) {
    width: 100%;
  }
