...
watch([type, status, asset, network], () => {
  resetPageAndCommit()
}, {flush: 'sync'})
...
watch(() => route.query.detail, (value) => {
  void openDetail(typeof value === 'string' ? value : null)
})
onMounted(() => {
  void load()
  ...
  <div class="page transactions-page">
    ...
    <AppCard padding="none">
      <div class="transaction-toolbar">
        <AppInput v-model="search" class="transaction-search" label="جست‌وجو" icon="search" type="search"
                  inputmode="search" placeholder="شماره پیگیری یا TXID…"/>
        <button class="mobile-filter-toggle" type="button" :aria-expanded="mobileFiltersOpen"
                aria-controls="transaction-advanced-filters" @click="mobileFiltersOpen = !mobileFiltersOpen">
          <AppIcon name="filter" :size="17"/>
          فیلترها<span v-if="hasActiveFilters">فعال</span></button>
        <div id="transaction-advanced-filters" class="transaction-filter-controls"
             :class="{ 'is-mobile-open': mobileFiltersOpen }">
          <AppSelect v-model="type" :options="transactionTypeOptions" label="نوع تراکنش" searchable
                     search-placeholder="جست‌وجوی نوع تراکنش"/>
          <AppSelect v-model="asset" :options="assetOptions" label="دارایی" searchable
                     search-placeholder="جست‌وجوی نام یا نماد دارایی"/>
          <AppSelect v-model="status" :options="transactionStatusOptions" label="وضعیت"/>
        </div>
      </div>
      <div class="date-range-toolbar" :class="{ 'is-mobile-open': mobileFiltersOpen }"
           aria-label="بازه زمانی تراکنش‌ها">
        <div class="date-range-heading">
          <AppIcon name="calendar" :size="19"/>
          <span><strong>بازه تاریخ شمسی</strong><small>تاریخ‌ها بر اساس زمان تهران محاسبه می‌شوند.</small></span></div>
        <AppInput v-model="fromDate" label="از تاریخ" icon="calendar" inputmode="numeric" ltr maxlength="10"
                  placeholder="۱۴۰۵/۰۶/۰۱" :error="parsedDateRange.fromError" @blur="normalizeDateField('from')"/>
        <AppInput v-model="toDate" label="تا تاریخ" icon="calendar" inputmode="numeric" ltr maxlength="10"
                  placeholder="۱۴۰۵/۰۶/۳۱" :error="parsedDateRange.toError" @blur="normalizeDateField('to')"/>
        <AppInput v-model="network" label="شبکه (اختیاری)" inputmode="text" ltr maxlength="20" placeholder="TRC20"
                  @blur="normalizeNetwork"/>
      </div>
      <div class="filter-summary"><span><AppIcon name="filter" :size="16"/> {{ totalResults.toLocaleString('fa-IR') }} نتیجه</span>
        <button v-if="hasActiveFilters" type="button" @click="clearFilters">پاک کردن فیلترها</button>
      </div>
      <div v-if="loading" class="skeleton">
        <AppSkeleton v-for="i in 6" :key="i" height="4.9rem"/>
      </div>
      <TransactionList v-else-if="transactions.length" :transactions="transactions"/>
      <EmptyState v-else-if="!error" icon="transactions" title="هنوز تراکنشی با این مشخصات ندارید"
                  description="فیلترها را تغییر دهید یا اولین واریز تومان خود را انجام دهید.">
        <RouterLink class="empty-action" to="/app/deposit/toman">واریز تومان</RouterLink>
      </EmptyState>
      <EmptyState v-else icon="warning" title="تراکنش‌ها دریافت نشد" :description="error">
        <button class="empty-action" type="button" @click="load">تلاش دوباره</button>
      </EmptyState>
    ...
    <AppModal :model-value="!!detail" title="جزئیات تراکنش" size="md" @update:model-value="!$event && closeDetail()">
      <template v-if="detail">
        <div class="detail-top"><span class="detail-icon"><AppIcon
            :name="detail.type.includes('deposit') ? 'arrowDown' : detail.type.includes('withdrawal') ? 'arrowUp' : 'trade'"
            :size="25"/></span>
          <div><span>{{ detail.title }}</span>
            <MoneyValue v-if="detail.assetSymbol === 'IRT'" :value="detail.amount"/>
            <CryptoValue v-else :value="detail.amount" :symbol="detail.assetSymbol"/>
          </div>
          <StatusBadge domain="transaction" :status="detail.status"/>
        </div>
        <dl class="transaction-details">
          <div>
            <dt>شماره پیگیری</dt>
            <dd>
              <bdi>{{ detail.referenceNumber }}</bdi>
              <CopyButton :value="detail.referenceNumber" label=""/>
            </dd>
          </div>
          <div v-if="detail.tomanAmount">
            <dt>ارزش تومان</dt>
            <dd>{{ formatToman(detail.tomanAmount) }}</dd>
          </div>
          <div v-if="detail.fee">
            <dt>{{ detail.networkCode ? 'کارمزد شبکه' : 'کارمزد' }}</dt>
            <dd>{{
                detail.assetSymbol === 'IRT' ? formatToman(detail.fee) : formatCrypto(detail.fee, {symbol: detail.assetSymbol})
              }}
            </dd>
          </div>
          <div v-if="detail.networkCode">
            <dt>شبکه</dt>
            <dd dir="ltr">{{ detail.networkCode }}</dd>
          </div>
          <div v-if="detail.address" class="technical">
            <dt>آدرس مقصد</dt>
            <dd>
              <bdi>{{ detail.address }}</bdi>
              <CopyButton :value="detail.address" label=""/>
            </dd>
          </div>
          <div v-if="detail.txId" class="technical">
            <dt>TXID</dt>
            <dd>
              <bdi>{{ detail.txId }}</bdi>
              <CopyButton :value="detail.txId" label=""/>
            </dd>
          </div>
          <div v-if="detail.confirmations !== undefined">
            <dt>تأییدهای شبکه</dt>
            <dd>{{ detail.confirmations?.toLocaleString('fa-IR') }} از
              {{ detail.requiredConfirmations?.toLocaleString('fa-IR') }}
            </dd>
          </div>
          <div>
            <dt>زمان ثبت</dt>
        ...
        <div v-if="detail.txId" class="network-note">
          <AppIcon name="info" :size="18"/>
          شناسه تراکنش برای پیگیری روی شبکه عمومی نمایش داده می‌شود.
        </div>
      </template>
      <template #footer>
        <button class="close-detail" type="button" @click="closeDetail">بستن</button>
        <RouterLink class="support-link" to="/app/support" @click="closeDetail">پیگیری از پشتیبانی</RouterLink>
      </template>
    </AppModal>
  </div>
</template>
...
.transaction-toolbar {
  display: grid;
  grid-template-columns: minmax(15rem, 1fr) minmax(33rem, 1.6fr);
  align-items: end;
  gap: var(--space-3);
  padding: var(--space-5);
}
...
.date-range-toolbar {
  display: grid;
  grid-template-columns: minmax(12rem, .8fr) repeat(3, minmax(10rem, 1fr));
  align-items: start;
  gap: var(--space-3);
...
.empty-action, .close-detail, .support-link {
  display: inline-flex;
  min-height: 2.65rem;
  align-items: center;
  justify-content: center;
...
.detail-top > div > :deep(.money-value strong), .detail-top > div > :deep(.crypto-value strong) {
...
@media (max-width: 767px) {
  ...
  .transaction-filter-controls.is-mobile-open {
    display: grid;
  }
  ...
  .date-range-toolbar.is-mobile-open {
    display: grid;
  }

  .date-range-heading, .date-range-toolbar > :last-child {
    grid-column: 1 / -1;
  }
...
@media (max-width: 560px) {
  .transaction-filter-controls, .date-range-toolbar {
