  ...
  if (
    ...
    coreError.value =
        reason instanceof Error
            ? reason.message
            : 'اطلاعات لازم برای معامله دریافت نشد.'
  }
...
<template>
  <div class="page trade-page">
    <PageHeader
        title="خرید و فروش مستقیم"
        description="بدون بازار معاملاتی؛ شما مستقیم از روشا می‌خرید یا به روشا می‌فروشید"
    >
      <template #actions>
        <div class="trade-header-actions">
          ...
          <span v-if="marketStore.lastUpdated" class="market-live">
        <i/>
        نرخ‌ها به‌روز هستند
      </span>
        </div>
    ...
    <div class="trade-layout">
      <OtcQuickTrade v-if="!loading && !coreError && walletStore.summary && marketStore.assets.length"
                     :key="`${initialSide}-${initialAsset}`" :assets="marketStore.assets" :wallet="walletStore.summary"
                     :initial-side="initialSide" :initial-asset="initialAsset"/>
      <AppSkeleton v-else-if="loading" height="42rem" radius="var(--radius-xl)"/>
      <AppCard v-else class="trade-error" padding="lg" role="alert"><span><AppIcon name="warning" :size="25"/></span>
        <div><strong>فرم معامله آماده نشد</strong>
          <p>{{ coreError || 'کیف پول یا نرخ ارزها در دسترس نیست.' }}</p></div>
      ...
      <aside class="trade-aside">
        <AppCard class="trust-card">
          ...
          <ul>
            <li>
              ...
              بدون صف خریدار و فروشنده
            </li>
            <li>
              <AppIcon name="check" :size="16"/>
              تأیید مبلغ نهایی پیش از ثبت
            </li>
            <li>
              <AppIcon name="check" :size="16"/>
              پیگیری سفارش از یک مسیر روشن
            </li>
          </ul>
        </AppCard>
        <AppCard v-if="verification">
          <div class="aside-title"><span><AppIcon name="info" :size="20"/></span>
            <div><h3>حدود معامله امروز</h3>
              <p v-if="level">بر اساس {{ level }} حساب شما</p></div>
          </div>
          <dl class="limits">
            <div>
              <dt>حداقل هر سفارش</dt>
              <dd>۱ میلیون تومان</dd>
            </div>
            <div>
              <dt>حداکثر هر سفارش</dt>
              <dd>۵۰۰ میلیون تومان</dd>
            </div>
            <div>
              <dt>باقی‌مانده خرید امروز</dt>
              <dd class="success">{{ formatToman(remainingBuy) }}</dd>
            </div>
            <div>
              <dt>باقی‌مانده فروش امروز</dt>
              <dd class="success">{{ formatToman(remainingSell) }}</dd>
            </div>
          </dl>
          <RouterLink to="/app/verification">مشاهده سطح و سقف‌ها
        ...
        <div class="support-tip">
          ...
          <span>برای معامله نیاز به راهنمایی دارید؟</span>
          <RouterLink to="/app/support">گفت‌وگو با پشتیبانی</RouterLink>
        </div>
      </aside>
    </div>
  </div>
...
.market-live i {
  ...
  box-shadow: 0 0 0 4px color-mix(in srgb, var(--color-success), transparent 86%);
}

.trade-layout {
  display: grid;
  grid-template-columns: minmax(28rem, 1.35fr) minmax(19rem, .65fr);
  align-items: start;
  gap: var(--space-5);
}
...
.trust-card {
  border-color: color-mix(in srgb, var(--color-primary), var(--color-border) 72%);
  background: linear-gradient(145deg, var(--color-surface-1), color-mix(in srgb, var(--color-primary-soft), var(--color-surface-1) 45%));
}
...
.trust-mark {
  ...
  width: 3.25rem;
  height: 3.25rem;
  border: 1px solid color-mix(in srgb, var(--color-primary), transparent 78%);
  border-radius: 1rem;
...
.trust-card li {
  ...
  background: color-mix(in srgb, var(--color-surface-2), transparent 24%);
  color: var(--color-text-secondary);
  font-size: var(--font-size-sm);
}

.trust-card li :deep(svg) {
  color: var(--color-success);
}
...
@media (max-width: 767px) {
  ...
  @media (max-width: 767px) {
    ...
    .trade-header-actions .market-live {
      display: none;
    }
