...
const load = async (force = false) => {
  ...
  if (ordersResult.status === 'fulfilled') {
    recentOrders.value = ordersResult.value.items
  } else {
    partialErrors.push(
        errorText(
            ordersResult.reason,
            'سفارش‌های اخیر دریافت نشدند.',
        ),
    )
  }
  ...
  if (transactionsResult.status === 'fulfilled') {
    ...
    partialErrors.push(
        errorText(
            ...
            'تراکنش‌های اخیر دریافت نشدند.',
        ),
    )
  }

  if (verificationResult.status === 'fulfilled') {
    verification.value = verificationResult.value
  } else {
    partialErrors.push(
        errorText(
            verificationResult.reason,
            'وضعیت احراز هویت دریافت نشد.',
        ),
    )
  }
  ...
  if (
    ...
    portfolioHistory.value = []
    portfolioHistoryLoading.value = false
    portfolioHistoryError.value = errorText(
        marketsResult.reason,
        'قیمت‌های تاریخی دارایی دریافت نشد.',
    )
  }
}
...
<template>
  <div class="page dashboard-page">
    <PageHeader
        title="داشبورد"
        description="تصویر روشن و به‌روز از دارایی‌ها و فعالیت حساب شما"
    >
      <template #actions>
    ...
    <AppCard v-if="showWelcome && verification" class="welcome-checklist" padding="md">
      <div class="welcome-copy"><span class="welcome-icon"><AppIcon name="sparkle" :size="24"/></span>
        <div><h2>حسابتان را برای اولین معامله آماده کنید</h2>
          <p>{{ welcomeMessage }}</p></div>
        <button type="button" aria-label="بستن راهنمای شروع" @click="dismissWelcome">
          <AppIcon name="close" :size="19"/>
        </button>
      </div>
      <div class="welcome-steps" :style="{ '--welcome-columns': String(Math.max(1, requiredSteps.length)) }">
        <RouterLink v-for="(step, index) in requiredSteps" :key="step.id" :to="stepRoute(step)"
                    :class="{ done: step.status === 'verified' }">
          <i>
            <AppIcon v-if="step.status === 'verified'" name="check" :size="14"/>
    ...
    <div v-else-if="walletStore.summary" class="dashboard-layout">
      <div class="dashboard-primary">
        ...
        <OtcQuickTrade v-if="marketStore.assets.length" class="area-trade" :assets="marketStore.assets"
                       :wallet="walletStore.summary" compact/>
        <AppCard v-else class="area-trade market-unavailable" padding="lg">
          <AppIcon name="warning" :size="24"/>
          <strong>نرخ‌های معامله در دسترس نیست</strong>
          <p>{{ marketStore.error || 'برای دریافت دوباره نرخ‌ها تلاش کنید.' }}</p>
      ...
      <AppCard v-if="verification" class="area-kyc account-ready" padding="md">
        ...
        <div><strong>{{ verificationTitle }}</strong>
          <p>{{ verification.message }}</p><span class="progress"><i
              :style="{ width: `${verification.progressPercent}%` }"/></span></div>
    ...
    <div v-if="walletStore.summary" class="dashboard-sections">
      <AppCard class="dashboard-preview-card" padding="none">
        <div class="card-heading">
          <div><h2>دارایی‌های من</h2>
            <p>خلاصه موجودی و ارزش روز دارایی‌ها</p></div>
          <RouterLink to="/app/wallet">مشاهده کیف پول
      ...
      <AppCard class="dashboard-preview-card" padding="none">
        <div class="card-heading">
          <div><h2>قیمت ارزها</h2>
            <p>قیمت خرید و تغییرات ۲۴ ساعت بازار</p></div>
          <RouterLink to="/app/markets">مشاهده همه ارزها
            <AppIcon name="chevronLeft" :size="16"/>
          </RouterLink>
        </div>
      ...
      <div class="activity-grid">
        <AppCard padding="none">
          <div class="card-heading">
            <div><h2>سفارش‌های اخیر</h2>
              <p>آخرین خرید و فروش‌های OTC</p></div>
            <RouterLink to="/app/orders">همه سفارش‌ها
              <AppIcon name="chevronLeft" :size="16"/>
            </RouterLink>
          </div>
        ...
        <AppCard padding="none">
          <div class="card-heading">
            <div><h2>تراکنش‌های اخیر</h2>
              <p>گردش مالی حساب شما</p></div>
...
.welcome-checklist {
  ...
  border-color: rgba(67, 139, 255, .2);
  background: linear-gradient(100deg, var(--color-primary-soft), var(--color-surface-1) 58%);
}
...
.welcome-steps {
  display: grid;
  grid-template-columns: repeat(var(--welcome-columns), minmax(0, 1fr));
  gap: var(--space-2);
...
.welcome-steps .done {
  border-color: rgba(53, 201, 149, .18);
...
.account-ready {
  ...
  align-items: center;
  gap: var(--space-4);
  border-color: rgba(221, 183, 110, .17);
  background: linear-gradient(100deg, var(--color-gold-soft), var(--color-surface-1) 55%);
}
...
.dashboard-sections > *, .activity-grid > * {
  min-width: 0;
}
...
.dashboard-skeleton {
  display: grid;
  grid-template-columns: 1fr minmax(26rem, .72fr);
  gap: var(--space-5);
}

.dashboard-skeleton > :last-child {
  grid-column: 2;
  grid-row: 1 / span 2;
}
...
.load-error {
  ...
  border: 1px solid rgba(240, 108, 117, .18);
  border-radius: var(--radius-md);
...
.dashboard-fatal strong {
  font-size: var(--font-size-lg);
}

.dashboard-fatal p, .market-unavailable p {
  margin: .15rem 0 0;
  color: var(--color-text-muted);
  font-size: var(--font-size-sm);
}
...
@container dashboard (max-width: 72rem) {
  ...
  .dashboard-overview, .area-trade, .area-kyc {
    grid-column: auto;
    grid-row: auto;
  }
...
@media (max-width: 767px) {
  ...
  .dashboard-primary, .dashboard-overview {
    gap: var(--space-4);
  }
  ...
  .account-ready .app-button {
    grid-column: 2;
    justify-self: start;
  }

  .area-actions :deep(.quick-actions) {
