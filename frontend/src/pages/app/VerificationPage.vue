...
import type {UserProfile, VerificationStep, VerificationSummary,} from '@/types'
...
const wizardCurrentStep = computed(() =>
    summary.value?.currentStepId
        ? summary.value.steps.find(
        (step) => step.id === summary.value?.currentStepId,
    ) ?? null
        : null,
)

const isWizardMode = computed(
    () => Boolean(wizardCurrentStep.value),
)
...
const wizardStepIndex = computed(() =>
    wizardCurrentStep.value
        ? Math.max(
            0,
            requiredSteps.value.findIndex(
                (step) => step.id === wizardCurrentStep.value?.id,
            ),
        )
        : 0,
)
...
const limitRows = computed<LimitRow[]>(() => {
  ...
  return [
    ...
    },
    {
      ...
      label: 'فروش روزانه',
      icon: 'arrowUp',
      limit: limits.dailySell,
      used: limits.usedSell,
    },
    {
      key: 'deposit',
      label: 'واریز تومان',
      icon: 'plus',
      limit: limits.dailyTomanDeposit,
      used: limits.usedTomanDeposit,
    },
    {
      key: 'withdraw',
      label: 'برداشت تومان',
      icon: 'minus',
      limit: limits.dailyTomanWithdrawal,
      used: limits.usedTomanWithdrawal,
    },
    {
      key: 'crypto',
      label: 'برداشت رمزارز',
      icon: 'wallet',
      limit: limits.dailyCryptoWithdrawalTomanEquivalent,
      used: limits.usedCryptoWithdrawalTomanEquivalent,
    },
  ]
})
...
async function handleStepSubmitted(
    message: string,
): Promise<void> {
