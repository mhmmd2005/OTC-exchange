...
function getService() {
  return isMockMode() ? mockAuthService : realAuthService
}
...
export const authService = {
  ...
  async verifyOtp(challengeId, otp) {
    return getService().verifyOtp(challengeId, otp)
  },

  async verifyLoginPassword(flowToken, password) {
    return getService().verifyLoginPassword(flowToken, password)
  },

  async registerWithPassword(flowToken, password, confirmPassword) {
    return getService().registerWithPassword(flowToken, password, confirmPassword)
  },

  async resetPassword(flowToken, password, confirmPassword) {
    return getService().resetPassword(flowToken, password, confirmPassword)
  },
  ...
  async refresh(refreshToken) {
    return getService().refresh(refreshToken)
  },
