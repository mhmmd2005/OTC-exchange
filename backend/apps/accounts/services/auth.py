...
class AuthService:
    ...
    def verify_otp(challenge_id, otp, request_ip=None, user_agent=""):
        ...
        submitted_hash = hash_otp(str(otp))
        ...
        return {"flow_token": flow_token, "next_step": "password", "expires_in": 600}
    ...
    def validate_flow_token(flow_token, expected_purpose=None):
        ...
        challenge = OTPVerification.objects.filter(id=payload.get("challenge_id")).first()
        if not challenge:
        ...
        if challenge.is_used is False or not challenge.verified_at:
